...
| #3 | GetFederationToken | IAM User's temporary security credentials <br> - Facebook Users can access AWS resources as if Facebook user were the IAM User.| 1. Already exists a long-term IAM user (=User-X) in AWS account. <br>2. GetSessionToken Request with the Access Key of User-X and IdP's JWT. <br>3. Check if the JWT is valid. <br>4. Provide User-X's temporary security credentials (ID/Key/Token). |
